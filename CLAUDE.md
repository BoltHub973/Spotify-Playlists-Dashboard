...
- Grid keyboard shortcuts (N/E/C/S defaults, plus THEME on ⌘⇧L) are stored in `localStorage` (`gridShortcuts.v1`); the native layer swallows only the sidebar shortcut (default ⌘S). CANCEL (C, and always Esc) leaves whichever mode is armed.
